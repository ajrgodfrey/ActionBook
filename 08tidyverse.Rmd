...
```
which takes a  random sample of 50 Star Wars characters who each have a defined species, and evaluates the mean height for each species, but only then keeps the species with more than one character and then finally prints them out in order from shortest to tallest.
...
```{r longChain2}
set.seed(123)
starwars %>% WhatIs() %>% filter(!is.na(species)) %>% sample_n(50) %>% group_by(species) %>% summarise(N = n(), MeanHeight = mean(height, na.rm=TRUE)) %>% CheckIt() %>% filter(N>1) %>% CheckIt() %>% arrange(MeanHeight) 
```
## BrailleR commands used in this chapter


The two main `BrailleR` functions relevant for use in the middle or at the end of a pipe chain are `WhatIs()` and `CheckIt()`.
